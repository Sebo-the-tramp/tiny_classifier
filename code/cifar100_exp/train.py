...
class ImageClassification(mm.MicroMind):
    ...
    def __init__(self, hparams, *args, **kwargs):
        ...
        if hparams.model == "phinet":
            ...
            self.modules["classifier"] = nn.Sequential(                
                nn.Linear(in_features=input_features, out_features=self.multiplier*hparams.num_classes)
            )
    ...
    def compute_loss(self, pred, batch):
        ...
        one_hot = torch.zeros(pred[1].shape[0], self.hparams.num_classes).to(pred[0].device)
        one_hot[torch.arange(pred[1].shape[0]), pred[1]] = 1           

        repeated = one_hot.repeat_interleave(self.multiplier, dim=1)        

        total = self.criterion(pred[0], repeated)
...
def top_k_accuracy(k=1):
    ...
    ### TODO define the top-k accuracy function given the multiplier and the number of classes
    ...
    def acc(pred, batch):
        ...
        reduced = pred[0].reshape(pred[0].size(0), -1, 5).sum(dim=-1)
        _, indices = torch.topk(reduced, k, dim=1)
