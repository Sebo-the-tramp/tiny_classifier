...
class ImageClassification(mm.MicroMind):
    ...
    def forward(self, batch):
        ...
        indices_1 = DiffSoftmax(x, tau=1.0, hard=False, dim=1)
        indices_int = torch.argmax(x, dim=1)

        broadcasted_bias = self.modifier_weights.unsqueeze(0).expand(len(batch[0]),-1,-1)
        
        out_expanded = indices_1.unsqueeze(2)        
        result = out_expanded * broadcasted_bias        
        weights = result.sum(dim=1)
        ...
        start = (indices_int* 10)
        end = (indices_int + 1) * 10
